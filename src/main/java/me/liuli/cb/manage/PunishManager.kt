...
class PunishManager {
    ...
    fun kickPlayerNoVL(
        name: String,
        reason: String = punishConfig.kickDefaultReason,
        operator: String = "Anticheat"
    ): Boolean {
        ...
        logger.warning("${player.name} kicked. (NO VL MODE)")
        return true
    }
...
//            banPlayer(data,reason = reason,operator = operator)
            ...
            punishConfig.banCommands.forEach {
                server.dispatchCommand(
                    AntiCheatCommandSender(operator), it.replace("%player%", player.name)
                        .replace("%playerIp%", player.address)
    ...
    fun checkPlayer(player: Player, data: Config): Boolean {
        ...
        if (data.getBoolean("banned")) {
            ...
            if (expired == -1L || expired > System.currentTimeMillis()) {
                ...
                // chain ban
                if (punishConfig.banIPChain && punishConfig.banWithIP) {
                    if (!ipExists(player.address)) {
                        banIP(player.address, punishConfig.banIPChainReason, expired, data["ban_operator"] as String)
                        logger.warning("${player.address} chain banned. Player: ${player.name}")
                    }
                }
        ...
        if (dataFile.exists()) {
            ...
            if (expired == -1L || expired > System.currentTimeMillis()) {
                ...
                if (punishConfig.banIPChain) {
                    banPlayer(
                        data,
                        reason = punishConfig.banIPChainReason,
                        time = expired,
                        operator = (ipData["ban_operator"] as String)
                    )
                    checkPlayer(player, data)
    ...
    fun banPlayer(
        name: String,
        reason: String = punishConfig.banDefaultReason,
        time: Long = punishConfig.banTime,
        operator: String = "Anticheat"
    ) {
    ...
    fun ipExists(ip: String): Boolean {
        return File(ipsFolder, "${ip.toLowerCase()}.yml").exists()
    }
